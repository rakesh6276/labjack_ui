...
def _loadLibrary():
    ...
    try:
        ...
        raise LabJackException(
            "Could not load labjackud driver. Ethernet connectivity availability only.\n\n    The error was: %s" % e)
    ...
    try:
        ...
        e = sys.exc_info()[1]
        raise LabJackException(
            "Could not load the %s for some reason other than it not being installed. Ethernet connectivity only.\n\n    The error was: %s" % (
            addStr, e))
...
class Device(object):
    ...
    def _writeToUE9TCPHandle(self, writeBuffer, modbus):
        ...
        if modbus is True:
            if self.handle.modbus is None:
                raise LabJackException(
                    "Modbus port is not available.  Please upgrade to UE9 Comm firmware 1.43 or higher.")
    ...
    def _parseReadRegisterResponse(self, response, numBytes, addr, format, numReg=None):
        ...
        if len(response) != numBytes:
            raise LabJackException(9001,
                                   "Got incorrect number of bytes from device. Expected %s bytes, got %s bytes. The packet recieved was: %s" % (
                                   numBytes, len(response), response))
    ...
    def _parseWriteRegisterResponse(self, response, request, value):
        ...
        if request[2] != 0 and request[3] != 0:
            ...
            raise Modbus.ModbusException(
                "Got an unexpected protocol ID: %s (expected 0). Please make sure that you have the latest firmware. UE9s need Comm Firmware of 1.50 or greater.\n\nThe packet you received: %s" % (
                protoID, hexWithoutQuotes(response)))

        if request[7] != response[7]:
            raise LabJackException(9002,
                                   "Modbus error number %s raised while writing to register. Make sure you're writing to an address that allows writes.\n\nThe packet you received: %s" % (
                                   response[8], hexWithoutQuotes(response)))
    ...
    def _checkCommandBytes(self, results, commandBytes):
        ...
        if len(results) == 0:
            ...
            hexWithoutQuotes(commandBytes), hexWithoutQuotes(results[1:(size + 1)]), hexWithoutQuotes(results)))
            ...
            self.deviceName, lowlevelErrorToString(results[6])))
    ...
    def open(self, devType, Ethernet=False, firstFound=True, serial=None, localId=None, devNumber=None, ipAddress=None,
        ...
        if self.handle is not None:
            raise LabJackException(9000,
                                   "Open called on a device with a handle. Please close the device, and try again. Your device is probably already open.\nLook for lines of code that look like this:\nd = u3.U3()\nd.open() # Wrong! Device is already open.")
    ...
    def streamStart(self):
        ...
        if results[2] != 0:
            raise LowlevelErrorException(results[2],
                                         "StreamStart returned an error:\n    %s" % lowlevelErrorToString(results[2]))
    ...
    if deviceType == 12:
        if U12DriverPresent():
            ...
            for i in range(pNumFound.value):
                deviceList[pSerialNumbers[i]] = {'SerialNumber': pSerialNumbers[i], 'Id': pIDs[i], 'ProdId': pProdID[i],
                                                 'powerList': pPowerList[i]}
    ...
    if _os_name == 'nt':
        ...
        for i in range(pNumFound.value):
            if pSerialNumbers[i] != 1010:
                deviceValue = dict(localId=pIDs[i], serialNumber=pSerialNumbers[i],
                                   ipAddress=DoubleToStringAddress(pAddresses[i]), devType=deviceType)
...
def _makeDeviceFromHandle(handle, deviceType):
    ...
    if deviceType == LJ_dtUE9:
        ...
        try:
            ...
            device.macAddress = "%02X:%02X:%02X:%02X:%02X:%02X" % (
            rcvDataBuff[33], rcvDataBuff[32], rcvDataBuff[31], rcvDataBuff[30], rcvDataBuff[29], rcvDataBuff[28])
...
def DoubleToStringAddress(number):
    ...
    number = int(number)
    address = "%i.%i.%i.%i" % (
    (number >> 8 * 3 & 0xFF), (number >> 8 * 2 & 0xFF), (number >> 8 & 0xFF), (number & 0xFF))
    return address
...
ERROR_TO_STRING_DICT['6'] = (
"SWDT_TIME_INVALID", "This error is caused when an invalid time was passed to the watchdog.")
ERROR_TO_STRING_DICT['7'] = ("XBR_CONFIG_ERROR", "")
ERROR_TO_STRING_DICT['16'] = (
"FLASH_WRITE_FAIL", "For some reason, the LabJack was unable to write the specified page of its internal flash.")
ERROR_TO_STRING_DICT['17'] = (
"FLASH_ERASE_FAIL", "For some reason, the LabJack was unable to erase the specified page of its internal flash.")
ERROR_TO_STRING_DICT['18'] = ("FLASH_JMP_FAIL",
                              "For some reason, the LabJack was unable to jump to a different section of flash. This may be an indication the flash is corrupted.")
...
ERROR_TO_STRING_DICT['23'] = ("FLASH_PAGE_NOT_IN_CODE_AREA",
                              "Usually, this error is raised when you try to write new firmware before upgrading the bootloader.")
...
ERROR_TO_STRING_DICT['40'] = ("SHT1x_COMM_TIME_OUT",
...
ERROR_TO_STRING_DICT['48'] = (
"STREAM_IS_ACTIVE", "This error is raised when you call StreamStart after the stream has already been started.")
ERROR_TO_STRING_DICT['49'] = ("STREAM_TABLE_INVALID", "")
ERROR_TO_STRING_DICT['50'] = ("STREAM_CONFIG_INVALID", "")
ERROR_TO_STRING_DICT['52'] = (
"STREAM_NOT_RUNNING", "This error is raised when you call StopStream after the stream has already been stopped.")
ERROR_TO_STRING_DICT['53'] = ("STREAM_INVALID_TRIGGER", "")
ERROR_TO_STRING_DICT['54'] = ("STREAM_ADC0_BUFFER_OVERFLOW", "")
ERROR_TO_STRING_DICT['55'] = ("STREAM_SCAN_OVERLAP",
                              "This error is raised when a scan interrupt is fired before the LabJack has completed the previous scan. The most common cause of this error is a configuration with a high sampling rate and a large number of channels.")
...
ERROR_TO_STRING_DICT['59'] = ("STREAM_AUTORECOVER_ACTIVE",
                              "This error is to inform you that the autorecover feature has been activated. Autorecovery is usually triggered by not reading data fast enough from the LabJack.")
ERROR_TO_STRING_DICT['60'] = ("STREAM_AUTORECOVER_REPORT",
                              "This error marks the packet as an autorecovery report packet which contains how many packets were lost.")
...
ERROR_TO_STRING_DICT['97'] = ("PIN_CONFIGURED_FOR_ANALOG",
                              "This error is raised when you try to do a digital operation on a pin that's configured for analog. Use a command like ConfigIO to set the pin to digital.")
ERROR_TO_STRING_DICT['98'] = ("PIN_CONFIGURED_FOR_DIGITAL",
...
ERROR_TO_STRING_DICT['102'] = (
"TC_PIN_OFFSET_MUST_BE_4-8", "This error is raised when you try to configure the Timer/Counter pin offset to be 0-3.")
...
class LJSocketHandle(object):
    ...
    def __init__(self, ipAddress, port, devType, firstFound, pAddress):
        try:
            ...
            e = sys.exc_info()[1]
            raise LabJackException(ec=LJE_LABJACK_NOT_FOUND,
                                   errorString="Couldn't connect to a LabJack at %s:%s. The error was: %s" % (
                                   ipAddress, port, str(e)))
...
def parseline(line):
    ...
    return {'prodId': int(prodId), 'crPort': crPort, 'modbusPort': modbusPort, 'spontPort': spontPort,
            'localId': int(localId), 'serial': int(serial)}
